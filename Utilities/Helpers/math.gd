...
static func spread(scale: float = 1.0) -> float:
	return (Random.randf() - 0.5) * 2.0 * scale
...
static func chance(probability_chance: float = 0.5, less_than: bool = true) -> bool:
	...
	return Random.randf() < probability_chance if less_than else Random.randf() > probability_chance
...
static func current_speed_on_miles_per_hour(velocity) -> float:
	if velocity is Vector2:
		return Random.roundf(velocity.length() * MetersPerSecondToMilePerHourFactor)
	elif velocity is Vector3:
		return Random.roundf(Vector3(velocity.x, 0, velocity.z).length() * MetersPerSecondToMilePerHourFactor)
	elif velocity is float: ## We assume we received the velocity length
		return Random.roundf(velocity * MetersPerSecondToMilePerHourFactor)
	else:
		return 0.0
...
static func current_speed_on_kilometers_per_hour(velocity) -> float:
	if velocity is Vector2:
		return Random.roundf(velocity.length() * MetersPerSecondToKilometersPerHourFactor)
	elif velocity is Vector3:
		return Random.roundf(Vector3(velocity.x, 0, velocity.z).length() * MetersPerSecondToKilometersPerHourFactor)
	elif velocity is float: ## We assume we received the velocity length
		return Random.roundf(velocity * MetersPerSecondToKilometersPerHourFactor)
	else:
		return 0.0
